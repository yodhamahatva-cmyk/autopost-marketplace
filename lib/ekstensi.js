...
import { periksaIklan, jumlahFoto, STATUS } from './iklan.js';
...
async function ping(req, setelan) {
  return {
    ...
    uji: !!setelan.modeUji, jeda: setelan.jedaMenit, batas: setelan.batasHarian,
...
async function ambil(req, setelan) {
  ...
  for (const iklan of antre) {
    ...
    await ubah(iklan, {
      ...
      keterangan: 'Sedang diposting oleh ekstensi Chrome' + (setelan.modeUji ? ' (MODE UJI — tidak diterbitkan)' : '') + '…'
    }, 'diproses');
    return {
      tugas: {
        id: iklan.id, token, uji: !!setelan.modeUji, versi: VERSI,
...
async function foto(req) {
  ...
  if (f.tipe === 'url') {
    ...
    return { nama: url.split('/').pop().split('?')[0] || 'foto.jpg', mime: res.headers.get('content-type') || 'image/jpeg', data: data.toString('base64') };
  }
