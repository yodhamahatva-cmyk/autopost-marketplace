...
export function rapikanIklan(x = {}) {
  ...
  return {
    ...
    akun: (x.akun || '').trim(),                 // '' = akun mana saja · '*' = semua akun · selain itu nama perangkat
    terbitAkun: x.terbitAkun && typeof x.terbitAkun === 'object' ? x.terbitAkun : {},
...
export function periksaIklan(iklan) {
  ...
  if (iklan.jenis === 'kendaraan') {
    ...
    // Formulir sepeda motor Facebook tidak punya kolom "Tipe body".
    if (!k.tipeBodi && !/motor/i.test(k.jenis || '')) kurang.push('Tipe bodi');
