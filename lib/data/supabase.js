...
/** Ubah galat Supabase menjadi pesan yang menjelaskan cara memperbaikinya. */
export function jelaskanGalat(galat, apa) {
  const m = String(galat?.message || galat || '');
  if (/could not find the table|relation .* does not exist|schema cache/i.test(m)) {
    return 'Tabel Supabase belum dibuat. Buka Supabase → SQL Editor, jalankan seluruh isi skema.sql, lalu muat ulang halaman. (' + apa + ')';
  }
  if (/could not find the .* column/i.test(m)) {
    return 'Struktur tabel Supabase tidak cocok dengan aplikasi (' + m + '). Jalankan ulang skema.sql versi terbaru. (' + apa + ')';
  }
  if (/invalid api key|jwt|unauthorized|401/i.test(m)) {
    return 'SUPABASE_SERVICE_ROLE_KEY salah atau bukan service role key. Periksa Project Settings → API di Supabase, perbarui variabel di Vercel, lalu Redeploy. (' + apa + ')';
  }
  if (/bucket not found/i.test(m)) {
    return 'Bucket foto "ap-foto" belum ada. Jalankan ulang skema.sql di SQL Editor Supabase. (' + apa + ')';
  }
  if (/fetch failed|enotfound|getaddrinfo/i.test(m)) {
    return 'Tidak bisa menghubungi Supabase — periksa SUPABASE_URL (harus https://xxxx.supabase.co). (' + apa + ')';
  }
  return 'Supabase (' + apa + '): ' + m;
}

const lempar = (galat, apa) => { if (galat) throw new Error(jelaskanGalat(galat, apa)); };

/** klienUji hanya untuk pengujian (tiruan Supabase). */
export function buatPenyimpananSupabase(klienUji) {
  if (klienUji) klien = klienUji;
  return {
    jenis: 'supabase',
    ...
    async daftarIklan({ status, cari, batas = 500 } = {}) {
      ...
      const aman = String(cari || '').replace(/[,()%*\\]/g, ' ').trim();
      if (aman) q = q.or(['judul.ilike.%' + aman + '%', 'kunci.ilike.%' + aman + '%'].join(','));
    ...
    async hapusIklan(id) {
      ...
      try {
        const { data: isi } = await sb().storage.from(EMBER).list(id, { limit: 200 });
        if (isi?.length) await sb().storage.from(EMBER).remove(isi.map((f) => id + '/' + f.name));
      } catch (e) { /* foto yatim tidak menghalangi penghapusan iklan */ }
      return true;
    },

    async tambahLog(baris) {
      const { error } = await sb().from(TABEL.log).insert({
        id: idBaru(), waktu: new Date().toISOString(),
        iklan_id: baris.iklanId || null, jenis: baris.jenis || 'info', pesan: baris.pesan || '', data: baris.data ?? null
      });
    ...
    async daftarLog(batas = 50) {
      ...
      return (data || []).map((r) => ({ id: r.id, waktu: r.waktu, iklanId: r.iklan_id, jenis: r.jenis, pesan: r.pesan, data: r.data }));
    },

    /** Daftar masalah konfigurasi (kosong = siap). Dipakai banner di dasbor. */
    async periksa() {
      const masalah = [];
      try { sb(); } catch (e) { return [e.message]; }
      for (const t of Object.values(TABEL)) {
        const { error } = await sb().from(t).select('*', { head: true, count: 'exact' }).limit(1);
        if (error) { masalah.push(jelaskanGalat(error, 'tabel ' + t)); break; }
      }
      if (!masalah.length) {
        const { error } = await sb().storage.getBucket(EMBER);
        if (error) masalah.push(jelaskanGalat(/not found/i.test(error.message) ? 'Bucket not found' : error, 'bucket ' + EMBER));
      }
      return masalah;
    },
...
const keBaris = (x) => ({
  ...
  hasil_url: x.hasilUrl, sumber: x.sumber, akun: x.akun || '', terbit_akun: x.terbitAkun || {},
  dibuat: x.dibuat, diubah: x.diubah
});
...
const dariBaris = (r) => ({
  ...
  akun: r.akun || '', terbitAkun: r.terbit_akun || {},
