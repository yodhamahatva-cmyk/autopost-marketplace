...
/**
 * Terapkan suntingan pratinjau ke hasil susunImpor: { <nomorBaris>: { akun, jadwal, ikut } }.
 * Sumbernya tidak diubah — hanya iklan yang akan disimpan.
 */
export function terapkanUbahan(hasil, ubahan) {
  const u0 = ubahan || {};
  return (hasil || []).map((h) => {
    const u = u0[h.baris] || {};
    const iklan = {
      ...h.iklan,
      ...(u.akun !== undefined ? { akun: u.akun } : {}),
      ...(u.jadwal ? { jadwal: u.jadwal } : {})
    };
    return { ...h, iklan, ikut: u.ikut !== false, disunting: u.akun !== undefined || !!u.jadwal };
  });
}
